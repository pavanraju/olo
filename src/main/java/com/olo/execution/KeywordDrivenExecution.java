...
public class KeywordDrivenExecution {
	
	private static final Logger logger = LogManager.getLogger(KeywordDrivenExecution.class.getName());
	private String testFile;
	private BrowserBot browser;
	private KeywordDrivenKeywords keywords;
	private HashMap<String,Object> level3FinalReport = new HashMap<String,Object>();
	
	public KeywordDrivenExecution(String testFile,BrowserBot browser,KeywordDrivenKeywords keywords){
	...
	private ArrayList<KeywordPropObject> readAndValidate(ITestContext ctx) throws Exception{
		...
		try {
			...
			File xlsFile=null;
			try {
				xlsFile = new File(KeywordDrivenExecution.class.getResource(testFile).toURI());
			} catch (Exception e) {
